...
def test_synthetic_generation_is_deterministic() -> None:
    ...
    assert first == second
    assert len(first) == 1_168
    assert sum(len(uplink.measurements) for uplink in first) == 3_345
...
def test_synthetic_data_contains_missing_and_invalid_cases() -> None:
    ...
    weather = [
        ...
        if uplink.external_device_id == "synthetic-v2-swale-weather-001"
    ]
    soil = [
        uplink for uplink in uplinks if uplink.external_device_id == "synthetic-v2-swale-soil-001"
    ]
    ...
    assert any(
        all(item.channel_code != "soil_moisture" for item in uplink.measurements) for uplink in soil
    )
    assert any(
